...
Archive version: 1.0.7 (July 21, 2026).

Author: Yuxuan Xu. Repository: https://github.com/xyx-ethan/rapid-sequential-touches-reanalysis. The archived release is https://doi.org/10.5281/zenodo.21482096; the all-version record is https://doi.org/10.5281/zenodo.21422472.
...
- `results/`: numerical outputs reported in the manuscript and supporting information, including observed-neighbor isolation, crossed baseline and response-window estimates, amplitude and phase interactions, protraction and retraction contrasts, recording-location summaries, contact-duration summaries, and label-level coefficient files.
...
The manuscript analysis retains layer 4 touches during the first 2 s after pole entry, excludes optogenetic-stimulation trials and missing phase, and uses archive subject labels for final aggregation. Cross-validation omits each set of complete trials from fitting once and then uses it to evaluate prediction. The reference phase threshold excludes a touch when another recorded onset falls within the threshold before or after it; at a trial boundary, the absence of a marker means only that no close onset was recorded. A stricter sensitivity requires observed within-trial neighbors on both sides. Phase is represented with first- and second-harmonic sine/cosine terms in the reference model. The archived phase is the argument of the analytic signal formed from the 6-60 Hz band-pass-filtered whisker base-angle series. Whisking amplitude is recomputed as the modulus of that analytic signal. The archived sign convention labels the negative and nonnegative phase half-cycles as protraction and retraction. These labels locate touches within the cycle; instantaneous direction is represented separately by velocity. Exact sign tests complement signed-rank tests for the reference phase comparisons, and regularization sensitivity covers 0.01, 0.1, 1, and 10. A continuous amplitude and phase analysis adds interactions between log amplitude and all four phase terms. Signed and absolute peak curvature changes after contact use sign(x)log(1 + 1,000|x|) and log(1 + 1,000|x|). Curvature and touch duration are analyzed in a separate coefficient model and are excluded from the predictive adjustment based on measurements before touch because they occur during the response interval. A descriptive sensitivity adds these measures after onset to the predictive adjustment. Additional outputs report direct contrasts between the phase-defined half-cycles and summaries for C2 and surrounding barrels.
